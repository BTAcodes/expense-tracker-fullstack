...
import { profileStyles } from "../assets/dummyStile";
...
const Profile = ({ user: initialUser, onUpdatedProfile, onLogout }) => {
...
const [user, setUser] = useState(
  ...
  const handleApiRequest = useCallback(
    async (method, endPoints, data = null) => {
  ...
  const handleCansleEdit = useCallback(() => {
    setTempUser(user);
    setEditMode(false);
  }, [user]);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      // reset password visiblitiy
      setShowPassword({ current: false, new: false, confirm: false });
    } catch (err) {
      toast.error(err.response?.data?.message || "faild to change password");
    }
  };
